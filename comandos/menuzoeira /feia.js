...
module.exports = {
    name: 'feia',
    category: 'menuzoeira',
    description: 'Mede o nível de feiura de uma usuária.',
    async execute(sock, msg, args, { from, reply }) {
        ...
        await reply("👽 Aguarde, analisando seu nível de feiura...");

        setTimeout(async () => {
            const porcentagem = Math.floor(Math.random() * 101);
            const foto = "./media/feia.jpg";
            
            let texto = `╭━━━〔 👽 *RADAR DE FEIURA* 👽 〕━━━╮\n┃\n`;
            texto += `┃  👤 *ALVA:* @${mencao.split('@')[0]}\n`;
            texto += `┃  👺 *FEIURA:* ${porcentagem}%\n`;
            texto += `┃\n`;
            texto += `┃  ⚠️ *ALERTA:* Meus circuitos estão derretendo!\n`;
            texto += `┃\n`;
            texto += `╰━━━━〔 🎖️ *ACKERMAN* 🎖️ 〕━━━━╯`;
